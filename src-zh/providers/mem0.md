...
| **默认 LLM** | GPT-4o-mini |
| **存储** | 向量数据库 + 内置实体链接（v3）；Neo4j 图已在 v3 中移除 |
...
Mem0 于 2026 年 5 月发布了从零重构的 v3 版本。最重大的变化是：**图记忆（Mem0g）已从开源 SDK 中移除**。原来对接 Neo4j/Memgraph/Kuzu/Apache AGE 的独立图驱动全部被拿掉，取而代之的是内置实体链接——在 `add()` 时自动提取实体并存入一个并行的向量集合。查询时，查询中的实体会给匹配的记忆加权排序，在不依赖外部基础设施的情况下达到与图变体相近的效果。

其他 v3 变更：
- **单次 ADD 提取**取代了原来的多轮 ADD/UPDATE/DELETE 循环，延迟大幅降低
- **多信号混合检索**将语义相似度、BM25 关键词匹配和实体图加权融合为单一评分
- **SDK 整理**：所有参数和响应字段统一为 camelCase；客户端初始化不再需要 `organizationId`/`projectId`
- **LongMemEval 得分跃升至 94.8%**（v1 为 66.9%），使 Mem0 跻身顶级系统之列

这是一次理念上的转变：Mem0 从「自带图数据库」转向了「内置实体链接已经够用」。对于此前支付 $249/月 Pro 图层级的团队而言，v3 开源版现在以零成本提供了可比的实体感知检索能力。

---

## 优势

- **经过大规模验证** — 57K+ GitHub stars 加上庞大的生产用户群，可靠性有底气。
...
- **v3 已移除图记忆** — 独立的 Neo4j/Memgraph/Kuzu/Apache AGE 图驱动已被移除；实体链接现在内置，但在复杂关系查询上不如完整图数据库灵活。
- **LOCOMO 得分有天花板（v3 之前）** — v1 基础变体得分 66.9%，不过 v3 架构已将 LongMemEval 推至 94.8%。
...
| 需要在实体密集领域用**实体感知检索**的团队 | v3 内置实体链接（无需外部图数据库） |
...
| 层级 | 价格 | 包含内容 |
|---|---|---|
| **开源版** | 免费 | v3 流水线 + 实体链接、本地向量数据库 |
| **Cloud Free** | $0 | 托管服务、用量有限 |
| **Pro** | $249/月 | 优先支持、更高配额（图层级已在 v3 中停用） |
